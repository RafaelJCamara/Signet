...
| Stop the registry, then re-run | An unhandled `HttpRequestException` before anything publishes — step 1 is plain REST, and registration belongs to CI (ADR-005), where a dead registry should fail the build loudly. The *SDK's* failure mode needs step 1 and the `WarmUpAsync` call commented out: under `FailOpen` publishes keep flowing and arrive with no envelope; under `FailClosed` the publish throws a `ConcordatException` naming the subject it could not resolve |
