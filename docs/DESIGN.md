...
| 023 | **No cross-subject references for Avro and Protobuf in v1** — registration fails with `schema_references_unsupported` | JSON Schema set a misleading precedent: its `$ref` is a URI with room for environment, subject and version. An Avro named-type reference is a bare fullname and a Protobuf `import` names a file — neither has anywhere to pin a version, so the reference could only bind to "whatever that subject holds now": ADR-017's failure reintroduced one layer down, in the reference graph. Same-document definitions (recursion included) and `google/protobuf/*` well-known imports still register. |
| 024 | **v1 ships the .NET SDK only; the Tier 2 SDKs are deferred, not cancelled** | The .NET SDK has never met a real workload, and writing four more SDKs against an unvalidated design multiplies whatever is wrong with it by five — each a public package with its own users to migrate. ADR-021 still stands: which four languages, and in what order. The interop groundwork — protocol docs, corpus, portability warnings — stays, and has already paid for itself with no second SDK in existence. |
| 025 | **A closed generic type is spelled `Outer_of_Arg`** | M2.3 refused generic type names on the grounds that any spelling becomes a rule five SDKs must reproduce identically. True, and the wrong conclusion: what had to be refused was *deriving* the spelling from one language's type system. The rule is defined over the outer and argument **names** in order — which every language with generics can produce — rather than over ``List`1[[Acme.Order, Asm, Version=…]]``, which only .NET can. Letting each SDK derive its own would give the same logical contract a different subject per language: a silent interop break, not a documented limitation. Open generics remain refused — they name no contract. |
