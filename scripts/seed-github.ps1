...
$labels = @(
    ...
    @{ Name = 'area:cli';        Color = '8250df'; Desc = 'concordat CLI and CI gate' }
...
$milestones = @(
    ...
    @{ Key = 'M2'; Title = 'M2 - .NET client + RabbitMQ.Client middleware'
       Desc = 'Concordat.Client with caching, the Concordat envelope, publish/consume middleware, quarantine. Exit: an invalid message is rejected to quarantine and not retried; the registry can be killed after warm-up without affecting delivery. docs/plan/M2-dotnet-client.md' }
    @{ Key = 'M3'; Title = 'M3 - CLI, CI gate, build-time packages'
       Desc = 'concordat CLI including infer, NativeAOT binaries, GitHub Action, MSBuild/Roslyn contract packages. Exit: a breaking change to a C# record fails the build and CI, naming the JSON-Pointer path. docs/plan/M3-cli.md' }
    ...
    @{ Key = 'M9'; Title = 'M9 - Concordat Cloud'
...
- [ ] NuGet: `Concordat`, `Concordat.Client`, `Concordat.Contracts`
- [ ] npm: the `@concordat` scope
- [ ] PyPI: `concordat-client`
- [ ] Go module path
...
'@
    ...
    # ------------------------------------------------------------------ M2
    New-Issue 'M2.1' 'M2' 'Concordat.Client' @('area:client','critical-path') @'
...
- [ ] **AMQP 1.0 conversion (ADR-013)** - confirm `concordat-*` headers arrive as application-properties, not message-annotations, when the same message is read by a 1.0 client. This is the only check that turns ADR-013 "designed to survive 1.0 conversion" from an assertion into a verified property.
...
'@
    ...
    # ------------------------------------------------------------------ M3
    New-Issue 'M3.1' 'M3' 'concordat CLI' @('area:cli','critical-path') @'
...
'@
    ...
    New-Issue 'M3.2' 'M3' 'concordat infer' @('area:cli','critical-path') @'
